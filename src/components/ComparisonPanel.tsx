...
import { computeMixAndMatchTotal, computeStoreTotals, formatZAR } from '../utils/compare'
...
export function ComparisonPanel({ basket, products }: ComparisonPanelProps) {
  const totals = useMemo(
    () => computeStoreTotals(basket, products, STORES).sort((a, b) => a.total - b.total),
    [basket, products],
  )
  ...
  const priciest = totals[totals.length - 1]
  const maxTotal = priciest.total || 1

  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {totals.map(({ store, total, missingItems }) => {
          const isCheapest = store.id === cheapest.store.id
          const barWidth = Math.max(6, (total / maxTotal) * 100)
          const diff = total - cheapest.total
          return (
            <div
              key={store.id}
            ...
            >
              ...
              {!isCheapest && diff > 0 && (
                <p className="mt-2 text-xs text-neutral-500">
                  +{formatZAR(diff)} vs cheapest
                </p>
              )}
              {missingItems.length > 0 && (
                <p className="mt-2 text-xs text-amber-600 dark:text-amber-500">
                  {missingItems.length} item{missingItems.length > 1 ? 's' : ''} not stocked
                  (excluded)
                </p>
              )}
            </div>
      ...
      <p className="text-xs text-neutral-500">
        Cheapest store ({cheapest.store.name}) is {formatZAR(priciest.total - cheapest.total)}{' '}
        less than the priciest ({priciest.store.name}) for this basket.
      </p>
    </div>
  )
}
